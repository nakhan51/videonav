...
def detectcircle(image):
    circles=cv2.HoughCircles(image, cv2.cv.CV_HOUGH_GRADIENT, 1, image.shape[0]/8,np.array([]),200, 12,5,10)
    return circles
...
while(cap.isOpened()):
    ...
    if(is_first_frame):
        ...
        if (circles_red is not None or circles_green is not None):
            ...
            if(inside>1): # rectangle was increased for search, so need to update reference.
                x1, y1, x2, y2, h, w, ref_pitch, ref_azimuth = update_reference(circles_red, circles_green, frame_no)
